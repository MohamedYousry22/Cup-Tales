...
begin
  -- Apple requires a fully functional review account. This account is shared
  -- only through App Store Connect review notes and may place test orders at
  -- any time. Normal customer accounts remain subject to business hours.
  if new.user_id = uuid 'a3ffad5e-025a-465f-811b-b15a9f84809a' then
    return new;
  end if;

  -- Preserve the one-off connected-device QA window until its existing expiry.
  if new.user_id = uuid '776ec61a-2d5a-437a-94a4-24afe61967ca'
     and clock_timestamp() < timestamptz '2026-08-02 05:00:00+03' then
    return new;
  end if;
